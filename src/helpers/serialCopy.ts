...
export class BrowserSerial {
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      console.log('Error releasing reader', error)
    }
    try {
      if (this.writeToStream) {
        // cancel the writer
        this.writeToStream.getWriter().releaseLock()
        await this.writeToStream.getWriter().close();
        // this.writeToStream.getWriter().abort()
        //   await this.outDone;
        this.writeToStream = null;
        //   this.outDone = null;
      }
    } catch (error) {
      console.log('error releasing writer', error)
    }
